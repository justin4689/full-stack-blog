...
const PostMenuActions = ({ post }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => axios.delete(`/posts/${post._id}`),
    onSuccess: () => { toast.success("Post deleted successfully!"); navigate("/"); },
    onError: (error) => toast.error(error.response.data),
  });

  const saveMutation = useMutation({
    mutationFn: () => axios.patch("/users/save", { postId: post._id }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["savedPosts"] }),
    onError: (error) => toast.error(error.response.data),
  });

  const featureMutation = useMutation({
    mutationFn: () => axios.patch("/posts/feature", { postId: post._id }),
  ...
  return (
    <div className="">
      ...
      {user && (post.user.username === user.username || isAdmin) && (
        <div className="flex items-center gap-2 py-2 text-sm cursor-pointer" onClick={() => deleteMutation.mutate()}>
